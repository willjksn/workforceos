...
export function PierOneMark({
  ...
  const gradientId = `pierone-public-mark-${tone}`;
  const start = tone === "onNavy" ? "#8FB3C0" : tone === "mono" ? "#6B7280" : "#4E7B8C";
  const end = tone === "onNavy" ? "#F4F7F8" : tone === "mono" ? "#6B7280" : "#0E2D4A";
...
export function Logo({
  ...
  const subColor = tone === "onNavy" ? "text-white/70" : "text-muted";

  if (variant === "icon") {
    return <PierOneMark className={className || "h-10 w-11"} tone={tone} title="PierOne Partners" />;
  }

  if (variant === "horizontal" && tone === "onLight") {
    return (
      <Image
        src="/brand/logo-lockup.png"
        alt="PierOne Partners"
        width={720}
        height={180}
        className={`h-[52px] w-auto max-w-[240px] object-contain object-left sm:h-[58px] sm:max-w-[280px] ${className}`}
        priority
        unoptimized
      />
    );
  ...
  if (variant === "stacked") {
    return (
      <div className={`flex flex-col items-center text-center ${className}`}>
        <PierOneMark className="h-12 w-[3.25rem]" tone={tone} />
        <p className={`mt-2 font-serif text-2xl font-semibold leading-none tracking-tight ${nameColor}`}>
          Pier<span className={oneColor}>One</span>
        </p>
        <p className={`mt-1.5 text-[10px] font-medium uppercase tracking-[0.32em] ${subColor}`}>Partners</p>
        {showTagline ? (
          <p className={`mt-2 text-[9px] font-medium uppercase tracking-[0.18em] ${subColor}`}>
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <PierOneMark className="h-10 w-11 shrink-0" tone={tone} />
      <div className="min-w-0 leading-tight">
        <p className={`font-serif text-[1.35rem] font-semibold tracking-tight ${nameColor}`}>
          Pier<span className={oneColor}>One</span>
        </p>
        <p className={`mt-0.5 text-[9px] font-medium uppercase tracking-[0.32em] ${subColor}`}>Partners</p>
        {showTagline ? (
          <p className={`mt-1 text-[8px] font-medium uppercase tracking-[0.16em] ${subColor}`}>
            Talent. Workforce. Opportunity.
          </p>
        ) : null}
      </div>
