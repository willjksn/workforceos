...
import { createReportExportSchedule } from "@/lib/reporting/schedules";
...
export async function scheduleReportExportAction(_prev: ActionState, formData: FormData): Promise<ActionState> {
  try {
    const principal = await requireAppPermission("reports.export");
    const parsed = z
      .object({
        category: z.string(),
    ...
    await createReportExportSchedule({
      principal,
      category: parsed.category,
    ...
    return { error: error instanceof Error ? error.message : "Unable to schedule report" };
  }
}
