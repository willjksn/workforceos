  ...
  const knowledgeSeed = [
    {
      ...
      content:
        "Professional & Technical Search (slug professional-search, DEC-SVC-005) is one of five launch services. Commercial path: inquiry → CRM → discovery → approved solution → proposal → contract (Direct Hire or Retained Search Agreement on the engagement) → delivery project. Search execution: structured job → internal Talent Network before external sourcing → job-specific scores (never a universal score, never auto-reject) → human pipeline → human-approved client submission → interview → offer → placement (fee/guarantee from the agreement) → silver medalists. Recruiter Standard does not own opportunities. Scout uses closed commands only, never SQL, never PII in prompts, and cannot send. Canonical markdown: docs/business/playbooks/professional-search.md. Cite approved professional-search service_workflows — do not invent scoring or fee rules.",
